...
function Calendar({ selected, onSelect, refreshKey }) {
    ...
    return (
        <div>
            <div className="h-[360px] rounded-2xl border border-stone-200 bg-white p-4 shadow-sm">        <DayPicker
                mode="single"
                selected={selected}
                onSelect={onSelect}
                weekStartsOn={1}
                ...
                classNames={{
                    ...
                    nav: 'flex gap-1',
                    button_previous: 'w-7 h-7 rounded-lg hover:bg-stone-100 transition-colors',
                    button_next: 'w-7 h-7 rounded-lg hover:bg-stone-100 transition-colors',
                }}
            />
            </div>
